...
\item Even low-intensity dark patterns cause large, persistent economic damage.
...
Platform economics track subscription revenue (scaled by a reputation factor $(R/100)^{0.5}$), dark pattern extraction revenue (with a $1.5\times$ multiplier for undetected exposures), and costs (churn replacement, support, and WOM damage). Platform reputation operates on a 0--100 scale with a floor of 2.0 and a natural cap of 92.0. Recovery mechanisms---customer support and natural trust recovery toward a harm-adjusted ceiling $\text{baseline}\times(1-\text{harm})$---are both suppressed by active dark pattern intensity.
...
The economic implications are stark. The control scenario generates cumulative revenue of \ctrlRev\ over six years; medium-intensity dark patterns reduce this to \medRev. While dark pattern extraction provides short-term gains, reputation collapse to the 2.0 floor sharply reduces per-user revenue and churn shrinks the active base. The opportunity cost---the gap between projected no-dark-pattern revenue and actual revenue---reaches \medOpp\ at medium intensity (Figure~\ref{fig:revenue}). Even at low intensity, the opportunity cost reaches \lowOpp, demonstrating that the ``safe'' use of mild dark patterns still generates substantial long-term economic damage, consistent with \citet{luguri2021}.
...
The results reveal a consistent dynamic: dark patterns create a self-reinforcing feedback loop. Trust erosion drives negative WOM, which erodes trust among unexposed users, which collapses reputation, which reduces revenue per user, which increases reliance on dark pattern extraction, which accelerates trust erosion further. Once deployed, the platform becomes dependent on extraction to maintain revenue, but extraction accelerates the very dynamics that destroy long-term value. This extends \citet{cara2019}'s observation that dark patterns ``work in the short term'' but ``lead to mistrust''; our model quantifies the magnitude of the resulting value destruction.
...
First, the non-linearity of outcomes suggests that intensity thresholds matter, so regulation that limits intensity may be more effective than binary bans. Second, the social-contagion dynamics show that dark patterns have externalities beyond the directly exposed user, strengthening the case for treating dark patterns as a public harm \citep{oecd2022}. Third, the economic analysis demonstrates that dark patterns are ultimately self-defeating---reducing total revenue while destroying reputation---which could support ``enlightened self-interest'' arguments in industry engagement.
